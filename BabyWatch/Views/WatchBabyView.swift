    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 10) {
                ...
                HStack {
                    Label(BabyFormat.time(settings.bedtimeDate), systemImage: "moon.fill")
                    Spacer()
                    Text(BabyFormat.compactMilligrams(health.bedtimeForecast.estimatedMilligrams))
                        .fontWeight(.semibold)
                }
                .font(.caption)
                .padding(8)
                ...
                ForEach(settings.quickAddDrinks) { drink in
                    Button {
                        ...
                        HStack {
                            ...
                            Spacer()
                            Text(BabyFormat.compactMilligrams(drink.milligrams))
                                .font(.caption2)
                        }
                    }
        ...
        ScrollView {
            VStack(spacing: 10) {
                ...
                Text(drink.name)
                    .font(.caption)
                    .foregroundStyle(Theme.textSecondary)
                Text(BabyFormat.milligrams(estimate))
                    .font(.title2.bold())
