...
private let watchSyncLogger = Logger(subsystem: "com.jackwallner.baby", category: "WatchSync")
...
struct WatchSettingsPayload: Sendable, Equatable {
    ...
    init?(context: [String: Any]) {
        bedtimeMinutes = context[babyBedtimeMinutesKey] as? Int
        halfLifeHours = context[babyHalfLifeKey] as? Double
        bedtimeThreshold = context[babyThresholdKey] as? Double
        quickAddPresets = context[babyPresetsKey] as? [Double]
        if let data = context[babyDrinkPresetsKey] as? Data {
            quickAddDrinks = try? JSONDecoder().decode([DrinkPreset].self, from: data)
        } else {
            quickAddDrinks = nil
        }
        isPro = context[babyCachedProKey] as? Bool
        hasCompletedSetup = context[babyHasCompletedSetupKey] as? Bool
        excludedSourceBundleIDs = context[babyExcludedSourcesKey] as? [String]
        excludedSourceNames = context[babyExcludedSourceNamesKey] as? [String: String]
    ...
    var dictionary: [String: Any] {
        ...
        if let bedtimeMinutes { value[babyBedtimeMinutesKey] = bedtimeMinutes }
        if let halfLifeHours { value[babyHalfLifeKey] = halfLifeHours }
        if let bedtimeThreshold { value[babyThresholdKey] = bedtimeThreshold }
        if let quickAddPresets { value[babyPresetsKey] = quickAddPresets }
        if let quickAddDrinks, let data = try? JSONEncoder().encode(quickAddDrinks) {
            value[babyDrinkPresetsKey] = data
        }
        if let isPro { value[babyCachedProKey] = isPro }
        if let hasCompletedSetup { value[babyHasCompletedSetupKey] = hasCompletedSetup }
        if let excludedSourceBundleIDs { value[babyExcludedSourcesKey] = excludedSourceBundleIDs }
        if let excludedSourceNames { value[babyExcludedSourceNamesKey] = excludedSourceNames }
        return value
    }
}
...
final class WatchSyncService: NSObject, WCSessionDelegate, @unchecked Sendable {
    ...
    private func apply(_ context: [String: Any]) {
        ...
        Task { @MainActor in BabySettings.shared.apply(payload) }
    }
    ...
    func session(
        ...
        #else
        Task { @MainActor in self.push(settings: BabySettings.shared.watchPayload) }
        #endif
    }
