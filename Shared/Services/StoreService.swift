...
enum RevenueCatConfig {
    ...
    /// Entitlement lookup key in RevenueCat project proj0b545ae2. The dashboard
    /// entitlement has lookup key `baby` and display name `Baby+`; `isPro` gates
    /// on any active entitlement, so a mismatch here would go unnoticed until
    /// someone reads this constant.
...
final class StoreService: NSObject, ObservableObject, PurchasesDelegate {
    ...
    /// gating.
    static let cachedProKey = babyCachedProKey
    ...
    @Published private(set) var isPro = false {
        didSet {
            ...
            WatchSyncService.shared.push(settings: BabySettings.shared.watchPayload)
    ...
    private let logger = Logger(subsystem: "com.jackwallner.baby", category: "Store")
    ...
    private static func fixtureProducts() -> [StoreProduct] {
        ...
        return [
            TestStoreProduct(
                localizedTitle: "Baby+ Monthly", price: 5.99, currencyCode: "USD",
                localizedPriceString: "$5.99", productIdentifier: BabyProduct.monthly,
            ...
            TestStoreProduct(
                localizedTitle: "Baby+ Yearly", price: 29.99, currencyCode: "USD",
                localizedPriceString: "$29.99", productIdentifier: BabyProduct.yearly,
            ...
            TestStoreProduct(
                localizedTitle: "Baby+ Lifetime", price: 59.99, currencyCode: "USD",
                localizedPriceString: "$59.99", productIdentifier: BabyProduct.lifetime,
