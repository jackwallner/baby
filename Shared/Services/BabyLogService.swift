...
@MainActor
final class BabyLogService: ObservableObject {
    static let shared = BabyLogService()
    ...
    func log(milligrams: Double, at date: Date = .now, drinkName: String? = nil) async -> Bool {
        ...
        let saved = await HealthKitService.shared.saveBaby(
        ...
        if let saved {
            ...
            let entry = LocalBabyEntry(date: date, milligrams: milligrams, drinkName: drinkName)
    ...
    func undoLast() async -> Bool {
        ...
        if let uuid = entry.healthKitUUID {
            removed = await HealthKitService.shared.deleteBaby(uuid: uuid)
    ...
    @discardableResult
    func delete(sample: BabySample) async -> Bool {
        guard sample.isOurs, let id = UUID(uuidString: sample.id) else { return false }
        let removed: Bool
        if sample.isLocalOnly {
            removed = deleteLocalEntry(id: id)
        } else {
            removed = await HealthKitService.shared.deleteBaby(uuid: id)
        }
        if removed {
    ...
    func retryPendingLocalEntries() async -> Int {
        ...
        let context = DataService.sharedModelContainer.mainContext
        let descriptor = FetchDescriptor<LocalBabyEntry>(predicate: #Predicate { $0.healthKitUUID == nil })
        var written = 0
        for entry in (try? context.fetch(descriptor)) ?? [] {
            if let uuid = await health.saveBaby(
    ...
    private func deleteLocalEntry(id: UUID) -> Bool {
        let context = DataService.sharedModelContainer.mainContext
        let descriptor = FetchDescriptor<LocalBabyEntry>(predicate: #Predicate { $0.id == id })
