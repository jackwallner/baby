...
/// Reads the Apple Health categories beyond dietary baby and turns them
/// into the comparisons the Body tab shows.
///
/// Deliberately separate from `HealthKitService`: baby read/write is the
...
final class HealthInsightsService: ObservableObject {
    ...
    private let logger = Logger(subsystem: "com.jackwallner.baby", category: "Insights")
    ...
    /// Asks for the body categories. Separate sheet from the baby request,
    ...
    func refresh(force: Bool = false) async {
        ...
        let settings = BabySettings.shared
        ...
        do {
            ...
            for offset in stride(from: Self.historyDays - 1, through: 0, by: -1) {
                ...
                let atBedtime = BabyClearance.remaining(
                    samples: doses,
                    at: bedtime,
                    selection: selection,
            ...
            var built = InsightsReport(
                ...
                heartRateResponse: BabyInsights.heartRateResponse(
                    doses: BabyClearance.included(samples: doses, selection: selection)
                        .filter { $0.endDate >= DateHelpers.daysAgo(Self.heartRateDays) },
                    heartRates: heartRates
                ),
                workouts: BabyInsights.workoutSummary(
                    workoutStarts: workoutStarts,
                    samples: doses,
                    selection: selection,
                    halfLifeHours: halfLife
                ),
                doseIntensity: BabyInsights.doseIntensity(
                    milligrams: records.last?.consumedMilligrams ?? 0,
                    bodyMassKilograms: bodyMass
                ),
                halfLifeSuggestion: BabyInsights.suggestedHalfLife(ageYears: ageYears),
                bodyMassKilograms: bodyMass,
                ageYears: ageYears,
    ...
    /// Groups sleep samples into nights keyed by the day whose baby came
