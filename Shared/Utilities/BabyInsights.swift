...
/// A body measurement Baby compares against baby exposure.
///
...
/// below (`HeartRateResponse`, `WorkoutBabySummary`, `DoseIntensity`,
...
enum BodyMetric: String, CaseIterable, Identifiable, Sendable {
    ...
    /// Overnight metrics describe the night after the baby day. Day metrics
    ...
    func formatted(_ value: Double) -> String {
        ...
        case .sleepDuration, .sleepOnset:
            return BabyInsights.durationText(value)
        case .sleepInterruptions:
    ...
    /// recorded rather than as an effect the baby caused.
    func differenceText(_ delta: Double) -> String {
        let magnitude = abs(delta)
        switch self {
        case .sleepDuration, .sleepOnset:
            return BabyInsights.durationText(magnitude)
        case .sleepInterruptions:
...
/// One baby day and the night that followed it.
struct DailyBodyRecord: Sendable, Equatable, Identifiable {
    var id: Date { date }
    /// Start of the day the baby was consumed.
    let date: Date
    /// Total dietary baby consumed that day, from the included sources.
    ...
    /// Overnight metrics are compared against the baby still modeled at
...
/// Higher-baby days measured against lower-baby days, for one metric.
struct MetricComparison: Sendable, Equatable, Identifiable {
    ...
    /// Higher-baby mean minus lower-baby mean.
...
struct WorkoutBabySummary: Sendable, Equatable {
...
struct InsightsReport: Sendable, Equatable {
    ...
    var cutoff: CutoffFinding = .insufficientData(have: 0, need: BabyInsights.minimumCutoffNights)
    var heartRateResponse: HeartRateResponse?
    var workouts: WorkoutBabySummary?
