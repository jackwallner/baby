...
/// Typical baby content for common drinks, used to seed presets and to give
...
enum DrinkCatalog {
    ...
    static let other = Category(name: "Other", drinks: [
        ...
        DrinkPreset(name: "Baby pill", milligrams: 100, symbolName: "pills.fill"),
    ])
    ...
    static func closestName(to milligrams: Double) -> DrinkPreset {
        ...
        guard let match, abs(match.milligrams - milligrams) <= 8 else {
            return DrinkPreset(name: "Baby", milligrams: milligrams, symbolName: "drop.fill")
        }
