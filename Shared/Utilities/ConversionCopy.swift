...
/// Pure copy helpers for Baby+ conversion CTAs. StoreKit always purchases
...
enum ConversionCopy {
    ...
    /// says the wait is expected and that Baby+ turns itself on.
    static let purchasePendingMessage =
        "Waiting on Apple to confirm this purchase. Baby+ turns on by itself as soon as it goes through, so there is nothing to tap again."
}
