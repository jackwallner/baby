...
final class BabyClearanceTests: XCTestCase {
    private let source = "com.jackwallner.baby"

    func testDoseHalvesAfterOneHalfLife() {
        XCTAssertEqual(
            BabyClearance.remaining(dose: 200, elapsedHours: 5, halfLifeHours: 5),
            100,
            accuracy: 0.001
        )
    }
    ...
    func testDoseQuartersAfterTwoHalfLives() {
        XCTAssertEqual(
            BabyClearance.remaining(dose: 200, elapsedHours: 10, halfLifeHours: 5),
            50,
            accuracy: 0.001
        )
    }
    ...
    func testExcludedSourceDoesNotContribute() {
        ...
        let selection = BabySourceSelection(excludedBundleIDs: [external])
        XCTAssertEqual(
            BabyClearance.remaining(
                samples: samples,
                at: now,
    ...
    func testOwnSourceCannotBeExcluded() {
        let now = Date(timeIntervalSince1970: 100_000)
        let selection = BabySourceSelection(excludedBundleIDs: [source])
        XCTAssertEqual(
            BabyClearance.remaining(
    ...
    func testForecastRangeUsesFourAndSixHourHalfLives() {
        ...
        let forecast = BabyClearance.forecast(
            samples: [sample],
            at: now,
            selection: .init(),
            halfLifeHours: 5
    ...
    func testTimeToThreshold() {
        let interval = BabyClearance.timeToReach(
    ...
    func testLatestTimeForDoseAccountsForExistingEstimate() throws {
        ...
        let result = BabyClearance.latestTimeForDose(
            dose: 100,
            existingSamples: [],
            bedtime: bedtime,
            threshold: 25,
    ...
    func testLatestTimeIsNilWhenExistingEstimateExceedsPreference() {
        ...
        XCTAssertNil(BabyClearance.latestTimeForDose(
            dose: 50,
    ...
    func testSourcesCollapsePhoneAndWatch() {
        ...
        let samples = [
            ...
            sample(120, at: now, bundleID: "com.jackwallner.baby.watch", isOurs: true),
        ]
        let rows = BabyClearance.sources(samples: samples, selection: .init())
    ...
    func testContributionsSumToRemainingAndRankByShare() {
        ...
        let entries = BabyClearance.contributions(
            samples: samples,
            at: now,
            selection: .init(),
            halfLifeHours: 5
    ...
    func testContributionsRespectExcludedSources() {
        ...
        var selection = BabySourceSelection()
