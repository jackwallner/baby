...
final class BabyInsightsTests: XCTestCase {

    // MARK: - Group comparison

    func testComparisonSplitsAtMedianAndReportsBothMeans() {
        // Twenty nights: the ten higher-baby ones slept an hour less.
        ...
        guard let comparison = BabyInsights.compare(records: records, metric: .sleepDuration) else {
    ...
    func testComparisonNeedsMinimumRecords() {
        let records = (0..<(BabyInsights.minimumRecords - 1)).map {
            record(daysAgo: $0, bedtime: Double($0) * 5, sleep: 7 * 3600)
        }
        XCTAssertNil(BabyInsights.compare(records: records, metric: .sleepDuration))
    }
    ...
    func testSmallDifferenceIsReportedButNotMeaningful() {
        ...
        let comparison = BabyInsights.compare(records: records, metric: .sleepDuration)
    ...
    func testDayMetricsCompareAgainstConsumedNotBedtime() {
        ...
        let comparison = BabyInsights.compare(records: records, metric: .steps)
    ...
    func testCutoffNeedsEnoughNights() {
        ...
        guard case let .insufficientData(have, need) = BabyInsights.personalCutoff(records: records) else {
            return XCTFail("Expected insufficientData")
        }
        XCTAssertEqual(have, 10)
        XCTAssertEqual(need, BabyInsights.minimumCutoffNights)
    }
    ...
    func testCutoffReportsNoDifferenceWhenSleepIsFlat() {
        ...
        guard case let .noMeasurableDifference(nights) = BabyInsights.personalCutoff(records: records) else {
    ...
    func testCutoffFindsTheThresholdWhereSleepDrops() {
        ...
        guard case let .found(cutoff) = BabyInsights.personalCutoff(records: records) else {
    ...
    func testCutoffRejectsLopsidedSplits() {
        ...
        guard case .noMeasurableDifference = BabyInsights.personalCutoff(records: records) else {
    ...
    func testCutoffIgnoresNightsWithoutSleep() {
        ...
        guard case let .insufficientData(have, _) = BabyInsights.personalCutoff(records: records) else {
            return XCTFail("Expected insufficientData")
        }
    ...
    func testPearsonIsOneForAPerfectLine() throws {
        let x = [1.0, 2, 3, 4, 5]
        let y = [2.0, 4, 6, 8, 10]
        XCTAssertEqual(try XCTUnwrap(BabyInsights.pearson(x, y)), 1, accuracy: 0.0001)
    }

    func testPearsonIsNilForAFlatSeries() {
        XCTAssertNil(BabyInsights.pearson([1, 1, 1, 1], [1, 2, 3, 4]))
    }
    ...
    func testHeartRateResponseComparesBeforeAndAfterEachDose() {
        ...
        let response = BabyInsights.heartRateResponse(doses: doses, heartRates: readings)
    ...
    func testHeartRateResponseSkipsDosesMissingEitherSide() {
        ...
        let response = BabyInsights.heartRateResponse(
            doses: [dose],
    ...
    func testHeartRateResponseIgnoresSmallDoses() {
        ...
        XCTAssertNil(BabyInsights.heartRateResponse(doses: doses, heartRates: readings))
    }
    ...
    func testWorkoutSummaryUsesTheModelAtEachStart() {
        ...
        let summary = BabyInsights.workoutSummary(
    ...
    func testWorkoutSummaryIsNilWithoutWorkouts() {
        XCTAssertNil(BabyInsights.workoutSummary(
            workoutStarts: [],
            samples: [],
            selection: .init(),
            halfLifeHours: 5
    ...
    func testSuggestedHalfLifeStaysInsideTheReferenceRange() {
        for age in [13, 25, 30, 49, 50, 64, 65, 90] {
            let suggestion = BabyInsights.suggestedHalfLife(ageYears: age)
            XCTAssertNotNil(suggestion, "age \(age)")
            let hours = suggestion?.hours ?? 0
            XCTAssertTrue(
                BabyClearance.referenceHalfLifeRange.contains(hours),
    ...
    func testSuggestedHalfLifeRisesWithAge() {
        let young = BabyInsights.suggestedHalfLife(ageYears: 25)?.hours ?? 0
        let older = BabyInsights.suggestedHalfLife(ageYears: 70)?.hours ?? 0
        XCTAssertLessThan(young, older)
    }

    func testSuggestedHalfLifeRejectsImplausibleAges() {
        XCTAssertNil(BabyInsights.suggestedHalfLife(ageYears: nil))
        XCTAssertNil(BabyInsights.suggestedHalfLife(ageYears: 4))
        XCTAssertNil(BabyInsights.suggestedHalfLife(ageYears: 130))
    }

    func testDoseIntensityNeedsABodyMass() {
        XCTAssertNil(BabyInsights.doseIntensity(milligrams: 200, bodyMassKilograms: nil))
        XCTAssertNil(BabyInsights.doseIntensity(milligrams: 200, bodyMassKilograms: 0))
        let intensity = BabyInsights.doseIntensity(milligrams: 200, bodyMassKilograms: 80)
    ...
    func testAnUnrecognisedAmountKeepsAGenericName() {
        ...
        XCTAssertEqual(preset.name, "Baby")
