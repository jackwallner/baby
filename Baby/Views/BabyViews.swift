...
struct PreviewRequest: Identifiable {
    ...
    var editing: BabySample?
}

struct BabyNowView: View {
    @EnvironmentObject private var settings: BabySettings
    @StateObject private var health = HealthKitService.shared
    @StateObject private var log = BabyLogService.shared
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showSettings) {
            NavigationStack { BabySettingsView() }
        }
        ...
        VStack(alignment: .leading, spacing: 18) {
            HStack(alignment: .firstTextBaseline) {
                VStack(alignment: .leading, spacing: 3) {
                    Text(BabyFormat.milligrams(health.remainingNow))
            ...
            HStack {
                VStack(alignment: .leading, spacing: 2) {
                    Text("AT BEDTIME")
                        .font(.caption2.bold())
                        .foregroundStyle(Theme.textSecondary)
                    Text(BabyFormat.milligrams(health.bedtimeForecast.estimatedMilligrams))
                        .font(.title2.bold())
                }
                Spacer()
                VStack(alignment: .trailing, spacing: 2) {
                    Text(BabyFormat.time(settings.bedtimeDate))
                        .font(.headline)
                    Text("4-6h range \(BabyFormat.range(health.bedtimeForecast))")
                        .font(.caption)
                        .foregroundStyle(Theme.textSecondary)
                }
            }
    ...
    private var contributions: [BabyContribution] {
        BabyClearance.contributions(
            samples: health.recentSamples,
            at: .now,
            selection: settings.sourceSelection,
            halfLifeHours: settings.halfLifeHours
    ...
    /// tapped anything, because Apple Health already held dietary baby from
    ...
    private func provenanceSummary(_ entries: [BabyContribution]) -> String {
        let count = entries.count
        let noun = count == 1 ? "entry" : "entries"
        var names: [String] = []
        for entry in entries {
            let name = entry.sample.isOurs ? "Baby" : entry.sample.sourceName
        ...
        if log.pendingWriteCount > 0 {
            VStack(alignment: .leading, spacing: 8) {
                ...
                Text(health.writeState == .denied
                    ? "Apple Health is not accepting writes from Baby. Turn on Baby under Health data in Settings, then retry."
                    : "They still count in every estimate here. Baby retries automatically each time you open the app.")
                    .font(.caption)
                    .foregroundStyle(Theme.textSecondary)
                HStack(spacing: 14) {
                    Button("Retry now") {
            ...
            ForEach(Array(settings.quickAddDrinks.enumerated()), id: \.offset) { _, drink in
                Button {
                    ...
                    VStack(spacing: 5) {
                        ...
                        Text(BabyFormat.milligrams(drink.milligrams))
                            .font(.caption2)
                            .foregroundStyle(Theme.textSecondary)
                    }
                ...
                .accessibilityLabel("Preview \(drink.name), \(BabyFormat.milligrams(drink.milligrams))")
            }
        }
    }
    ...
    private var previewButton: some View {
        Button {
            preview = PreviewRequest(drink: DrinkPreset(name: "Baby", milligrams: 100, symbolName: "drop.fill"))
        } label: {
            HStack {
    ...
    private var undoRow: some View {
        if let entry = log.lastEntry,
           ...
           Date.now.timeIntervalSince(loggedAt) < Self.undoWindow {
            HStack {
                Text("Logged \(entry.drinkName ?? "baby"), \(BabyFormat.milligrams(entry.milligrams))")
                    .font(.subheadline)
                    .foregroundStyle(Theme.textPrimary)
                Spacer()
                Button("Undo") {
        ...
        HStack {
            VStack(alignment: .leading, spacing: 4) {
                Text("CONSUMED TODAY")
                    .font(.caption2.bold())
                    .foregroundStyle(Theme.textSecondary)
                Text(BabyFormat.milligrams(health.consumedToday))
                    .font(.title.bold())
            }
            Spacer()
                        ...
                        HStack(spacing: 12) {
                            ...
                            VStack(alignment: .leading, spacing: 1) {
                                Text(sample.displayName)
                                    .fontWeight(.semibold)
                                    .foregroundStyle(Theme.textPrimary)
                                Text(BabyFormat.milligrams(sample.milligrams))
                                    .font(.caption)
                                    .foregroundStyle(Theme.textSecondary)
                            }
                            Spacer()
                            Text(BabyFormat.time(sample.endDate))
                                .foregroundStyle(Theme.textSecondary)
                            Image(systemName: "chevron.right")
                                .font(.caption2)
                                .foregroundStyle(Theme.textSecondary)
...
/// anything, because Apple Health already holds dietary baby written by
...
struct RemainingBreakdownSheet: View {
    ...
    let contributions: [BabyContribution]
    ...
    private var totalCard: some View {
        VStack(alignment: .leading, spacing: 4) {
            Text(BabyFormat.milligrams(total))
        ...
        VStack(alignment: .leading, spacing: 14) {
            ForEach(contributions) { entry in
                HStack(alignment: .top, spacing: 12) {
                    ...
                    Spacer(minLength: 8)
                    Text(BabyFormat.milligrams(entry.remainingMilligrams))
    ...
    private func subtitle(for entry: BabyContribution) -> String {
        let stamp = "\(BabyFormat.milligrams(entry.sample.milligrams)) at \(BabyFormat.time(entry.sample.endDate))"
    ...
    private var outsideSourceNote: some View {
        VStack(alignment: .leading, spacing: 6) {
            Label("Some of this arrived from another app", systemImage: "arrow.triangle.2.circlepath")
                .font(.subheadline.weight(.semibold))
                .foregroundStyle(Theme.textPrimary)
            Text("Baby reads dietary baby that anything else already wrote to Apple Health, which is why the estimate can be non-zero before you log anything here. Settings › Apple Health sources controls which of them count.")
                .font(.caption)
                .foregroundStyle(Theme.textSecondary)
        }
        .padding(16)
...
struct DrinkPreviewSheet: View {
    ...
    private var proposedForecast: BabyForecast {
        BabyClearance.forecastAdding(
            dose: dose,
            at: drinkTime,
            samples: baselineSamples,
    ...
    private var latestTime: Date? {
        BabyClearance.latestTimeForDose(
            dose: dose,
            ...
            ScrollView {
                VStack(spacing: 18) {
                    ...
                    if let editing = request.editing {
                        Button(role: .destructive) {
                            Task {
                                _ = await BabyLogService.shared.delete(sample: editing)
                                dismiss()
                            }
                        } label: {
                    ...
                    Text("Logging is free. The estimate uses your selected half-life and is not a measure of baby in your bloodstream.")
                        .font(.caption)
                        .foregroundStyle(Theme.textSecondary)
    ...
    private var forecastCard: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            Label(
                "This drink adds about \(BabyFormat.milligrams(doseContribution)) to the bedtime estimate",
            ...
            if let latestTime {
                Label(
                    latestTime >= settings.bedtimeDate
                        ? "At this size, the estimate stays under your \(BabyFormat.milligrams(settings.bedtimeThreshold)) preference right through bedtime"
                        : "Latest time this dose still lands under your preference: \(BabyFormat.time(latestTime))",
                ...
                Label(
                    "Your existing bedtime estimate is already above your \(BabyFormat.milligrams(settings.bedtimeThreshold)) preference",
                    systemImage: "exclamationmark.triangle.fill"
                )
                .font(.subheadline)
                .foregroundStyle(Theme.warning)
            ...
            Text("Reference range at bedtime: \(BabyFormat.range(proposedForecast)). A preference is a number you chose, not a safety limit.")
                .font(.caption)
                .foregroundStyle(Theme.textSecondary)
        }
        .padding(20)
    ...
    private func forecastValue(_ label: String, _ value: Double) -> some View {
        VStack(alignment: .leading, spacing: 3) {
            ...
            Text(BabyFormat.milligrams(value))
                .font(.title2.bold())
                .foregroundStyle(Theme.textPrimary)
    ...
    private var logButton: some View {
        Button {
            ...
            Task {
                if let editing = request.editing {
                    _ = await BabyLogService.shared.update(
                        sample: editing,
                        milligrams: dose,
                        at: drinkTime,
                        drinkName: drinkName
                    ...
                    _ = await BabyLogService.shared.log(
                        milligrams: dose,
                        at: drinkTime,
                        drinkName: drinkName
                    )
...
struct BabyTimelineView: View {
    ...
    @State private var history: [BabyDaySummary] = []
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showUpgrade) {
            BabyPaywallView(paywallImpressionID: "baby_timeline", focus: .fullHistory)
        }
    }
    ...
    private var lockedRange: some View {
        VStack(spacing: 12) {
            ...
            Text("\(days) days is part of Baby+")
                .font(.headline)
                .foregroundStyle(Theme.textPrimary)
            Text("The free timeline covers the last \(Self.freeDays) days. Baby+ opens the full \(Self.ranges.last ?? 90) days of intake and bedtime estimates.")
                .font(.subheadline)
                .foregroundStyle(Theme.textSecondary)
                .multilineTextAlignment(.center)
            Button("See Baby+") { showUpgrade = true }
                .font(.subheadline.weight(.semibold))
                .foregroundStyle(Theme.cyan)
        }
        .frame(maxWidth: .infinity)
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            ZStack {
                if isLoading && history.isEmpty {
                    ...
                    VStack(spacing: 6) {
                        Text("No baby recorded in this range")
                            .font(.subheadline.weight(.semibold))
                            .foregroundStyle(Theme.textPrimary)
                        Text("Logged drinks, and anything else writing dietary baby to Apple Health, show up here.")
                            .font(.caption)
                            .foregroundStyle(Theme.textSecondary)
                            .multilineTextAlignment(.center)
                    }
...
struct BabyCurve: View {
    let samples: [BabySample]
    let start: Date
    let end: Date
    ...
    var body: some View {
        GeometryReader { geometry in
            let values = (0...32).map { index -> Double in
                ...
                return BabyClearance.remaining(
                    samples: samples,
                    at: date,
                    selection: selection,
        ...
        .accessibilityLabel("Estimated baby clearance curve through bedtime")
    }
}
...
private extension Array where Element == BabyDaySummary {
    func average(_ keyPath: KeyPath<BabyDaySummary, Double>) -> Double {
