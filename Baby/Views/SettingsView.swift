...
struct BabySettingsView: View {
    ...
    var body: some View {
        ...
        .sheet(item: $upgradeFocus) { focus in
            BabyPaywallView(paywallImpressionID: "baby_settings_\(focus.rawValue)", focus: focus)
        }
    ...
    private var forecastSection: some View {
        Section("Forecast") {
            ...
            Stepper(
                "Bedtime preference: \(BabyFormat.milligrams(settings.bedtimeThreshold))",
                value: $settings.bedtimeThreshold,
                in: 5...100,
                step: 5
            )
            Text("The FDA describes a typical baby half-life of about 4 to 6 hours. Yours is adjustable because clearance varies from person to person. The bedtime preference is a number you chose, not a safety limit.")
                .font(.caption)
                .foregroundStyle(.secondary)
        }
    }
    ...
    private var appleHealthSection: some View {
        Section {
            ...
            if health.writeState == .denied {
                Text("Baby cannot write to Apple Health right now, so new entries are being kept on this device until access is restored.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            ...
            NavigationLink("Included sources") { BabySourcesView() }
            Text("Your entries are saved as dietary baby in Apple Health, marked as manually entered and tagged with the drink name. Deleting one here deletes it there too.")
                .font(.caption)
                .foregroundStyle(.secondary)
        } header: {
            Text("Apple Health")
    ...
    private var quickLogSection: some View {
        Section {
            ...
            if !store.isPro {
                Button("Customize with Baby+") { upgradeFocus = .customDrinks }
            } else {
    ...
    private var subscriptionSection: some View {
        Section("Baby+") {
            ...
            if store.isPro {
                ...
                Button("See Baby+") { upgradeFocus = .bodyComparisons }
            }
    ...
    private var aboutSection: some View {
        Section("About") {
            ...
            Link("Rate Baby", destination: AppStoreReviewLinks.writeReviewURL)
            ...
            Text("Baby provides mathematical estimates for personal awareness. It is not medical advice, not a measurement of baby in your body, and it does not diagnose, treat, or prevent any condition.")
                .font(.caption)
                .foregroundStyle(.secondary)
        }
    }
        ...
        NavigationStack {
            Form {
                Section("Custom") {
                    ...
                    Stepper(
                        BabyFormat.milligrams(milligrams),
                        value: $milligrams,
                        in: 1...600,
                        step: 5
                    )
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        ...
                        onSelect(DrinkPreset(
                            name: name.trimmingCharacters(in: .whitespaces).isEmpty ? "Baby" : name,
...
struct BabySourcesView: View {
    @EnvironmentObject private var settings: BabySettings
    @StateObject private var health = HealthKitService.shared

    private var sources: [BabySourceStatus] {
        let live = BabyClearance.sources(samples: health.recentSamples, selection: settings.sourceSelection)
        let liveIDs = Set(live.map(\.bundleID))
        let stored = settings.excludedSourceNames.compactMap { id, name -> BabySourceStatus? in
    ...
    var body: some View {
        List {
            Section {
                Text("All included dietary baby samples contribute once. Your entries from Baby always remain included.")
                    .font(.callout)
                    .foregroundStyle(.secondary)
            }
            Section("Sources from the last 48 hours") {
                ForEach(sources) { source in
                    Toggle(isOn: Binding(
                        ...
                        VStack(alignment: .leading) {
                            Text(source.name)
                            Text("\(BabyFormat.milligrams(source.milligrams)) from \(source.sampleCount) entries")
                                .font(.caption)
                                .foregroundStyle(.secondary)
                        }
                    }
...
struct ReviewPromptSheet: View {
    ...
    private func primaryAction() {
        ...
        case .feedback:
            ...
            dismiss()
            #if os(iOS)
            UIApplication.shared.open(BabyLinks.support)
            #endif
        }
    }
}
