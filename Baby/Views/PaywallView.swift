...
struct BabyPaywallView: View {
    ...
    var body: some View {
        ZStack(alignment: .topTrailing) {
            ...
            // that is titled Baby+ and is the only place to reach Manage
            // subscription.
            if store.isPro {
                subscriberContent
    ...
    ///
    /// This used to end with an "everything in Baby+" card repeating all six
    ...
    private var headline: String {
        ...
        return "See how baby lands\non your own body"
    }

    private var subhead: String {
        "What sits under your cutoff: baby next to the sleep and heart data already in Apple Health. The cutoff itself, logging and both estimates stay free."
    }
    ...
    private var disclosureText: String {
        ...
        if selected.babyPackageKind == .lifetime {
            return "\(selected.babyPriceLabel). One-time purchase, no subscription and nothing renews. \(Self.wellnessHedge)"
        }
    ...
    private func savingsPercent(for package: Package) -> Int? {
        guard package.babyPackageKind == .yearly,
              let monthly = store.packages.first(where: { $0.babyPackageKind == .monthly }) else {
            return nil
        }
...
/// The Baby+ feature list, driven off one enum so the paywall bullets and
...
enum PlusFeature: String, CaseIterable, Identifiable {
    ...
    var title: String {
        switch self {
        case .bodyComparisons: "Baby against sleep, heart rate and HRV"
    ...
    var detail: String {
        switch self {
        case .bodyComparisons:
            "Higher-baby days next to lower ones for time asleep, resting heart rate, HRV, breathing, blood oxygen and activity."
        case .tunedHalfLife:
    ...
    var pitchLine: String {
        switch self {
        case .bodyComparisons: "Baby next to your sleep, heart rate and HRV"
        case .tunedHalfLife: "A half-life starting point for your body"
    ...
    var pitchHeadline: String {
        switch self {
        case .bodyComparisons: "See baby next to\nyour sleep and heart data"
