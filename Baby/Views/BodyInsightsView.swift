...
/// Baby measured against the body data already in Apple Health.
///
...
struct BodyInsightsView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 18) {
                if !settings.bodyInsightsEnabled || !insights.isAuthorizationRequested {
                    ...
                    // distinguishes Baby could not be reached without a
                    // purchase. The comparisons behind it are still Baby+.
                    cutoffCard
                    if store.isPro {
                        comparisonSection
                        doseResponseSection
        ...
        .sheet(item: $upgradeFocus) { focus in
            BabyPaywallView(paywallImpressionID: "baby_body_\(focus.rawValue)", focus: focus)
        }
    }
    ...
    private var connectCard: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            Text("The half-life model is the same for everyone. Your sleep is not. With read-only access to the sleep and heart data already in Apple Health, Baby can line your higher-baby days up against your lower ones and report what was actually recorded.")
                .font(.subheadline)
    ...
    private var cutoffCard: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            case let .insufficientData(have, need):
                VStack(alignment: .leading, spacing: 8) {
                    Text("\(have) of \(need) nights")
                        ...
                        .foregroundStyle(Theme.textPrimary)
                    Text("Keep logging. Once there are \(need) nights with both baby and recorded sleep, Baby looks for the bedtime estimate above which your own sleep ran shorter.")
                        .font(.subheadline)
                        .foregroundStyle(Theme.textSecondary)
            ...
            case let .found(cutoff):
                VStack(alignment: .leading, spacing: 8) {
                    Text(BabyFormat.milligrams(cutoff.milligrams))
                        .font(.system(size: 42, weight: .bold, design: .rounded))
                        .foregroundStyle(Theme.textPrimary)
                    Text("On nights when your estimate at bedtime was at or above this, your recorded sleep averaged \(BabyInsights.durationText(cutoff.sleepDelta)) shorter than on nights below it.")
                        .font(.subheadline)
                        .foregroundStyle(Theme.textPrimary)
                    ...
                    if abs(cutoff.milligrams - settings.bedtimeThreshold) >= 5 {
                        Button("Use \(BabyFormat.milligrams(cutoff.milligrams)) as my bedtime preference") {
            ...
            Text("An association in your own records, not a safe limit and not a recommendation. Plenty of things other than baby change how you sleep.")
                .font(.caption2)
                .foregroundStyle(Theme.textSecondary)
        }
        .padding(22)
        ...
        if overnight.isEmpty && daytime.isEmpty {
            ...
            if !overnight.isEmpty {
                comparisonCard(
                    ...
                    caption: "Nights split at \(BabyFormat.milligrams(overnight[0].splitMilligrams)) estimated at bedtime.",
            ...
            if !daytime.isEmpty {
                comparisonCard(
                    ...
                    caption: "Days split at \(BabyFormat.milligrams(daytime[0].splitMilligrams)) consumed.",
                    comparisons: daytime
                )
            }
        }
    ...
    private var emptyComparisonCard: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text("Not enough overlapping days yet")
                .font(.headline)
                .foregroundStyle(Theme.textPrimary)
            Text("A comparison needs at least \(BabyInsights.minimumRecords) days that have both a baby total and the measurement, with \(BabyInsights.minimumPerGroup) days on each side of your median. Small samples produce confident-looking noise, so Baby waits.")
                .font(.subheadline)
                .foregroundStyle(Theme.textSecondary)
        }
        .padding(20)
            ...
            VStack(alignment: .leading, spacing: 14) {
                ...
                if let workouts = insights.report.workouts {
                    insightRow(
                        ...
                        value: BabyFormat.milligrams(workouts.averageOnBoard),
                        detail: "Across \(workouts.workoutCount) workouts, typically starting around \(BabyInsights.clockText(minutesAfterMidnight: workouts.typicalStartMinutes))."
                    )
                }
            }
            .padding(20)
            ...
            VStack(alignment: .leading, spacing: 14) {
                ...
                if let intensity = report.doseIntensity, intensity.milligrams > 0 {
                    insightRow(
                        ...
                        detail: "\(BabyFormat.milligrams(intensity.milligrams)) against \(Int(intensity.bodyMassKilograms.rounded())) kg from Apple Health."
                    )
                }
    ...
    ///
    /// The verdict itself is free: it is the answer no other baby app can
    /// give, so hiding it hid the reason to want the app at all. What is still
    /// Baby+ is everything that sits underneath the verdict, and it is
    ...
    /// invented: the numbers under the blur are the ones Baby+ reveals.
    private var blurredFindings: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            Text("These are your own numbers, blurred until Baby+.")
                .font(.caption)
                .foregroundStyle(Theme.textSecondary)
        }
        .padding(22)
    ...
    private var whatItAnswersCard: some View {
        VStack(alignment: .leading, spacing: 14) {
            Label("Baby+", systemImage: "sparkles")
                .font(.caption.bold())
                .foregroundStyle(Theme.violet)
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                questionRow("waveform.path.ecg", "Do my higher-baby days show up in resting heart rate or HRV?")
            ...
            Text("Baby answers each from your own days, or says there is not enough overlap yet. It will not manufacture a finding out of a small sample.")
                .font(.caption)
                .foregroundStyle(Theme.textSecondary)
                .fixedSize(horizontal: false, vertical: true)
        }
        ...
        if !empty.isEmpty {
            VStack(alignment: .leading, spacing: 6) {
                Text("No data for \(empty.map(\.title).joined(separator: ", "))")
                    ...
                    .foregroundStyle(Theme.textPrimary)
                Text("Either nothing has been recorded for those, or they were left off when you granted access. Health › Sharing › Apps controls what Baby can read.")
                    .font(.caption2)
                    .foregroundStyle(Theme.textSecondary)
    ...
    private var disclaimer: some View {
        Text("Baby describes what Apple Health already recorded alongside your intake. It does not diagnose, treat, or prevent anything, and it is not a substitute for advice from a clinician.")
            .font(.caption)
            .foregroundStyle(Theme.textSecondary)
...
private struct ComparisonRow: View {
    ...
    private var detail: String {
        ...
        return "\(lower) on lower-baby days, \(higher) on higher ones (\(counts))."
    }
}
